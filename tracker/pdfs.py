...
def build_quote_pdf(project, quote, output_path):
    ...
    class QuotePDF(FPDF):
        ...
        def header(self):
            ...
            self.cell(98, 6, sanitize_pdf_text(self.project_name))
            self.set_font("Helvetica", "", 8.5)
            self.set_text_color(*MUTED)
            self.cell(48, 6, sanitize_pdf_text(self.quote_number), align="C")
            self.cell(40, 6, sanitize_pdf_text(self.quote_date), align="R")
            self.ln(10)
    ...
    scope_text_h = sum(wrapped_height(paragraph, 174, 4.3) for paragraph in scope_paragraphs)
    scope_h = 10 + scope_text_h + (len(scope_paragraphs) - 1) * 1.4 + 6
    scope_y = pdf.get_y()
    pdf.rect(12, scope_y, 186, scope_h, style="DF")
    pdf.set_xy(18, scope_y + 5)
    pdf.set_text_color(*INK)
    ...
    for index, paragraph in enumerate(scope_paragraphs):
        pdf.multi_cell(174, 4.3, sanitize_pdf_text(paragraph))
...
def build_ldm_pdf(project, ldm, output_path):
    ...
    pdf.cell(width, 10, "LISTA DE MATERIALES", ln=True, align="C")
    pdf.set_font("Helvetica", "", 11)
    pdf.set_text_color(79, 142, 247)
    pdf.cell(width, 6, sanitize_pdf_text(ldm.get("ldm_number", "")), ln=True, align="C")
    pdf.ln(5)

    pdf.set_text_color(0, 0, 0)
    quote_number = ldm.get("cot_proveedor", "") or "-"
    info_rows = [
        ("Proyecto:", sanitize_pdf_text(project.get("name", ""))),
        ("Proveedor:", sanitize_pdf_text(ldm.get("proveedor", "") or "-")),
        ("Fecha:", sanitize_pdf_text(ldm.get("fecha", ""))),
        ("Cot. proveedor:", sanitize_pdf_text(quote_number)),
    ]
    for label, value in info_rows:
        pdf.set_font("Helvetica", "B", 9)
        pdf.cell(40, 6, label)
        pdf.set_font("Helvetica", "", 9)
        pdf.cell(width - 40, 6, value, ln=True)
    pdf.ln(5)

    with_prices = any(item.get("precio_cot", 0) for item in ldm.get("items", []))
    if with_prices:
        columns = [10, 74, 18, 18, 30, 30]
        headers = ["#", "Descripción", "Unidad", "Cant.", "P. Unit. Cot.", "Total Cot."]
        aligns = ["C", "L", "C", "C", "R", "R"]
    else:
        columns = [12, 108, 30, 30]
        headers = ["#", "Descripción", "Unidad", "Cantidad"]
        aligns = ["C", "L", "C", "C"]

    pdf.set_fill_color(26, 31, 46)
    pdf.set_text_color(255, 255, 255)
    pdf.set_font("Helvetica", "B", 9)
    for col_width, header, align in zip(columns, headers, aligns):
        pdf.cell(col_width, 7, header, fill=True, align=align)
    pdf.ln()

    pdf.set_text_color(0, 0, 0)
    pdf.set_font("Helvetica", "", 9)
    fill = False
    for index, item in enumerate(ldm.get("items", []), 1):
        pdf.set_fill_color(248, 250, 252) if fill else pdf.set_fill_color(255, 255, 255)
        if with_prices:
            price = float(item.get("precio_cot", 0))
            total = float(item.get("total_cot", 0))
            values = [
                str(index),
                sanitize_pdf_text(item.get("description", ""))[:55],
                sanitize_pdf_text(item.get("unit", "")),
                str(item.get("qty", "")),
                f"${price:,.2f}",
                f"${total:,.2f}",
            ]
            ...
            values = [
                str(index),
                sanitize_pdf_text(item.get("description", ""))[:75],
                sanitize_pdf_text(item.get("unit", "")),
                str(item.get("qty", "")),
            ]
        for col_width, value, align in zip(columns, values, aligns):
            pdf.cell(col_width, 6, value, fill=fill, align=align)
        pdf.ln()
        fill = not fill

    if with_prices:
        pdf.ln(3)
        subtotal = float(ldm.get("subtotal_cot", 0))
        left = sum(columns[:-1])
        right = columns[-1]
        pdf.set_font("Helvetica", "B", 10)
        pdf.set_text_color(26, 31, 46)
        pdf.cell(left, 7, "SUBTOTAL COTIZADO", align="R")
        pdf.set_text_color(34, 197, 94)
        pdf.cell(right, 7, f"${subtotal:,.2f}", align="R", ln=True)

    pdf.ln(6)
    pdf.set_font("Helvetica", "I", 8)
    pdf.set_text_color(150, 150, 150)
    note = "Documento de cotización de proveedor." if with_prices else "Documento para cotización - no incluye precios."
    pdf.cell(width, 5, note, align="C", ln=True)

    if ldm.get("notes"):
        pdf.ln(4)
        pdf.set_text_color(0, 0, 0)
        pdf.set_font("Helvetica", "B", 9)
        pdf.cell(width, 5, "Notas:", ln=True)
        pdf.set_font("Helvetica", "", 9)
        pdf.multi_cell(width, 5, sanitize_pdf_text(ldm["notes"]))
