...
def build_quote_pdf(project, quote, output_path=None):
    ...
    def add_signature_section():
        block_h = 20
        _sig_bottom = pdf.h - 15.5   # 1mm sobre la línea del footer (~pdf.h-14.5)
    ...
    _cover_mode = "reducido" if _portada_spacing <= 24 else "normal"
    ...
    # ── Términos: preparación (usada por Reducido inline y por página separada en Normal) ──
    ...
    scope_h = (_scope_pad + 8.0) + scope_text_h + (len(scope_paragraphs) - 1) * _scope_pad + _scope_pad
    scope_y = pdf.get_y()
    pdf.rect(pdf.l_margin, scope_y, content_width, scope_h, style="DF")
    pdf.set_xy(scope_inner_left, scope_y + _scope_pad)
    pdf.set_text_color(*INK)
    pdf.set_font("DejaVu", "B", 15.0)
    pdf.cell(0, 8.0, "Alcance", ln=True)
